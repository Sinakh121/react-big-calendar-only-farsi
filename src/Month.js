...
class MonthView extends React.Component {
  ...
  renderHeaders(row) {
    ...
    let res = dates.range(first, last, 'day')

    //fix weekday headers
    function checkDay(index){
      return res[index].toString().slice(0,4)
    }
    res = res.filter((day, idx) => {
      if(idx < res.length - 1){
        return checkDay(idx) !== checkDay(idx + 1)
      }
      return true
    })
    if( checkDay(res.length - 1) === checkDay(res.length - 2) ) res.pop();
    //

    return res.map((day, idx) => (
